...
def load_and_process_hub_n1(gc, spreadsheet):
    ...
    if not is_loading:
        ...
        header = data[0]
        col_id_idx, col_name_idx, col_total_idx, col_ret_idx, col_am_idx = 1, 2, 4, 5, 6

        for idx, cname in enumerate(header[:7]):
            cn = str(cname).strip()
            if cn == 'ID bưu cục': col_id_idx = idx
            elif cn == 'Tên bưu cục': col_name_idx = idx
            elif cn == 'Total đơn': col_total_idx = idx
            elif cn == 'Đơn return': col_ret_idx = idx
        ...
        region_col = df_fd.columns[0]
        id_col     = df_fd.columns[1]
        bc_col     = df_fd.columns[2]
        tot_col    = df_fd.columns[4]
        ret_col    = df_fd.columns[5]
