...
for r_idx, r in enumerate(data):
    for c_idx, c in enumerate(r.get('values', [])):
        ...
        if val:
            print(f"R{r_idx+1}C{c_idx+1} '{val}': bg={fmt.get('backgroundColor')}, fg={fmt.get('textFormat', {}).get('foregroundColor')}, bold={fmt.get('textFormat', {}).get('bold')}")
