...
def build_lm_status(lm_data):
    """Tự động tìm dòng header và trích xuất trạng thái đơn hàng từ tab data LM."""
    lm_header_idx = -1
    order_col_idx = -1
    status_col_idx = -1
    ...
    for idx, row in enumerate(lm_data[:10]):
        ...
        if order_col_idx != -1 and status_col_idx != -1:
            lm_header_idx = idx
            break

    if lm_header_idx == -1:
        raise ValueError("Không tìm thấy cột 'Mã đơn hàng' và 'Trạng thái' trong tab data LM.")
...
def compute_current_aging_15(aging_data, lm_status, cocau_map):
    """Tính danh sách đơn aging > 15 ngày TẠI THỜI ĐIỂM HIỆN TẠI (tự động nhận diện cột)."""
    aging_header_idx = -1
    ag_order_col = -1
    ag_bc_col = -1
    ag_id_bc_col = -1
    ...
    if aging_header_idx == -1:
        raise ValueError("Không tìm thấy đủ cột bắt buộc trong tab Đơn giao aging trên 5 ngày.")
