...
output_path = os.path.join(workspace_dir, "scratch", "calculate_opr_vols_res.txt")

with open(output_path, "w", encoding="utf-8") as f:
    df = pd.read_excel(file_path, sheet_name="OPR")
    f.write(f"OPR Columns: {list(df.columns)}\n")
    f.write(f"OPR Shape: {df.shape}\n")
    
    # Check date column
    # Let's parse NgayLTC as datetime
