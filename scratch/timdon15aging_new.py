...
def get_gspread_client(spreadsheet_id=SHEET_KEY):
    ...
    seen_creds = set()
    cred_paths = []
    for p in raw_cred_paths:
        norm_p = os.path.normpath(os.path.abspath(p)).lower() if p else ""
        if norm_p and norm_p not in seen_creds:
            seen_creds.add(norm_p)
            cred_paths.append(p)
    ...
    seen_auth = set()
    auth_user_paths = []
    for p in raw_auth_user_paths:
        norm_p = os.path.normpath(os.path.abspath(p)).lower() if p else ""
        if norm_p and norm_p not in seen_auth:
            seen_auth.add(norm_p)
            auth_user_paths.append(p)
...
def get_baseline_path(date_str):
    """Mỗi ngày một file riêng, không ghi đè lẫn nhau."""
...
def main():
    ...
    # 2. Extract backlog status from data LM (tự động tìm dòng header)
    lm_header_idx = -1
    lm_order_col = -1
    lm_status_col = -1

    for idx, row in enumerate(lm_data[:10]):
        norm_cells = [unicodedata.normalize('NFC', str(c)).strip().lower() for c in row]
        for col_idx, cell in enumerate(norm_cells):
            if cell in ['mã đơn hàng', 'ma don hang', 'mã đơn', 'ma don', 'order_code']:
                lm_order_col = col_idx
            elif cell in ['trạng thái', 'trang thai', 'status']:
                lm_status_col = col_idx
        if lm_order_col != -1 and lm_status_col != -1:
            lm_header_idx = idx
            break

    if lm_header_idx == -1:
        print("❌ Lỗi: Không tìm thấy cột Mã đơn hàng / Trạng thái trong tab data LM.")
        sys.exit(1)
    ...
    # 3. Process Đơn giao aging trên 5 ngày (tự động tìm dòng header và vị trí cột)
    aging_header_idx = -1
    ag_order_col = -1
    ag_bc_col = -1
    ag_id_bc_col = -1
    ...
    for idx, row in enumerate(aging_data[:10]):
        norm_cells = [unicodedata.normalize('NFC', str(c)).strip().lower() for c in row]
        for col_idx, cell in enumerate(norm_cells):
            if cell in ['order_code', 'mã đơn hàng', 'ma don hang', 'mã đơn', 'ma don']:
                ag_order_col = col_idx
            elif cell in ['bc', 'bưu cục', 'buu cuc']:
                ag_bc_col = col_idx
            elif cell in ['id_bc', 'warehouse_id', 'id bưu cục']:
                ag_id_bc_col = col_idx
            elif cell in ['aging']:
                ag_aging_col = col_idx
            elif cell in ['nhóm bl', 'nhom bl', 'nhóm backlog']:
                ag_group_col = col_idx
            elif cell in ['am_name', 'am']:
                if ag_am_col == -1 or cell == 'am_name':
                    ag_am_col = col_idx

        if all(c != -1 for c in [ag_order_col, ag_bc_col, ag_id_bc_col, ag_aging_col, ag_group_col, ag_am_col]):
            aging_header_idx = idx
            break

    if aging_header_idx == -1:
        print("❌ Lỗi: Không tìm thấy đủ cột bắt buộc trong tab Đơn giao aging trên 5 ngày.")
        sys.exit(1)
    ...
    for row in aging_data[aging_header_idx + 1:]:
    ...
    # 3.5. Lưu baseline riêng cho ngày hôm nay (KHÔNG ghi đè baseline của ngày khác)
    today_ymd = datetime.now().strftime('%Y-%m-%d')
    baseline_file = get_baseline_path(today_ymd)
    ...
    if os.path.exists(baseline_file) and not force_reset:
        ...
        print(f"ℹ️ Baseline cho ngày hôm nay ({today_ymd}) đã tồn tại tại {baseline_file}. "
              f"Bỏ qua ghi đè để tránh làm giảm baseline trong ngày. Dùng --force để reset.")
    ...
    if write_baseline:
        ...
        try:
            ...
            print(f"✅ Đã lưu baseline đơn aging > 15 ngày cho ngày {today_ymd} tại {baseline_file}")
        except Exception as e:
