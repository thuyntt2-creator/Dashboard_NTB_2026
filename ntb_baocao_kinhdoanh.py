"""
NTB DAILY BUSINESS REPORT – Google Sheets Writer
==================================================
Đọc data từ Google Sheet → phân tích → ghi kết quả vào 4 sheet mới có màu

Yêu cầu:
    pip install gspread google-auth pandas

Chạy:
    python ntb_baocao_kinhdoanh.py
"""
...
def print_full_report(D, nhom_data=None):
    ...
    lines.append("")
    lines.append(f"2. Doanh thu Khách hàng mới (KHM) ngày {d.strftime('%d/%m')}: {F(sdt)}")
    lines.append(f"   + {P(sdt,sdp)} so với ngày N-1 ({sp} shop – {F(sdp)})")
    lines.append(f"   + {P(sdt,sd7)} so với cùng kỳ tuần trước ({F(sd7)})")
    lines.append(f"   - KHM luỹ kế tháng {m.month}: {sm} shop – {M(sdm)}")
    lines.append(f"     + {P(sdm,sdmp)} so với cùng kỳ tháng {pm.month} ({smp} shop – {M(sdmp)})")
    lines.append("")
    ...
    lines.append("   - Thúc đẩy chỉ tiêu phát triển KH mới (KHM) trong nửa cuối tháng để bù đắp khoảng trống lũy kế (MTD)")
    lines.append("")
