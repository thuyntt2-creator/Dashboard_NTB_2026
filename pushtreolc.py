...
def load_stuck_orders(sh, bc_map):
    """
    Tải tất cả các đơn treo luân chuyển từ các tab AM riêng lẻ trong Google Sheet
    (đã được Google Sheet lọc sẵn các đơn đọng >= 24h theo từng AM).
    Trả về: { am_name: { bc_name: [ {"code": ma_don, "group": grp, "ton_h": ton_val}, ... ] } }
    """
    EXCLUDE_TABS = {"stuck", "pivot", "cơcấu", "cơ cấu", "lognhacnho", "1", "2", "khôngxácđịnh"}
    ...
    am_worksheets = [
        w for w in all_ws 
        if w.title.strip().lower().replace(" ", "").replace("_", "") not in EXCLUDE_TABS
    ]

    # Ưu tiên đọc từ các tab AM riêng lẻ (như tab Lê Thanh Nhựt, Phạm Bá Thành Công,...)
    if am_worksheets:
        for ws in am_worksheets:
            ...
            am_name = tab_name  # Tên AM chính là Tên Tab

            for row in values[1:]:
                if len(row) <= max(i_madon, i_status, i_ton, i_bc):
                    continue
    ...
    # Fallback đọc tab 'stuck' nếu không có tab AM riêng
